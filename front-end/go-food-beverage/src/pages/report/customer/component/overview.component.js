...
export default function OverviewComponent(props) {
  ...
  const getDataFilterTopCustomer = () => {
    let startDate = moment(selectedDates?.startDate).format(DateFormat.YYYY_MM_DD_HH_MM_SS);
    let endDate = moment(selectedDates?.endDate).add(1, "d").seconds(-1).format(DateFormat.YYYY_MM_DD_HH_MM_SS);
