...
import { formatGoogleAddress } from "constants/google.constant";
  ...
  const onSetFormAddressValue = (address) => {
    if (address) {
      if (isDefaultCountry) {
        ...
        const mainAddress =
          splitAddress.length === formatGoogleAddress.defaultLength
            ? splitAddress[0]
            : splitAddress[0].concat(", ", splitAddress[1]);
        const city = splitAddress[splitAddress.length === formatGoogleAddress.defaultLength ? 3 : 4];
        const district = splitAddress[splitAddress.length === formatGoogleAddress.defaultLength ? 2 : 3];
        const fullWard = splitAddress[splitAddress.length === formatGoogleAddress.defaultLength ? 1 : 2];
