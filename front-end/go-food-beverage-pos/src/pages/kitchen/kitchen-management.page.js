...
export function KitchenManagementPage(props) {
  ...
  const printStamp = (orderId, orderItemId, type) => {
    if (autoPrintStamp) {
      ...
      orderDataService.printOrderStampDataAsync(printStampRequest).then((responseData) => {
        if (responseData) {
          ...
          if (stampRef && stampRef.current) {
            stampRef.current.render(stampConfig, stampData, type === printStampType.ALL_ORDER_ITEMS ? true : false);
  ...
  const onClickOrderItem = (session, orderItem, itemIndex, sessionIndex) => {
    ...
    printStamp(session?.orderId, orderItem?.orderItemId, printStampType.ORDER_ITEM);
  };
