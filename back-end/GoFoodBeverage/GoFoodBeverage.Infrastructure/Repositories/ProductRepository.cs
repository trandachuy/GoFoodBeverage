...
namespace GoFoodBeverage.Infrastructure.Repositories
{
    public class ProductRepository : GenericRepository<Product>, IProductRepository
    {
        ...
        {
            var product = dbSet.FirstOrDefaultAsync(p => p.Name.Trim().ToLower() == productName.Trim().ToLower() && p.StoreId == storeId);
        ...
        {
            var product = dbSet.FirstOrDefaultAsync(p => p.Name.Trim().ToLower() == productName.Trim().ToLower() && p.StoreId == storeId && p.StatusId == (int)EnumStatus.Active && p.IsActive);
        ...
        {
            var products = dbSet.Where(s => s.StoreId == storeId && s.StatusId == (int)EnumStatus.Active);
        ...
        {
            var product = dbSet.Where(s => s.StoreId == storeId && s.Id == id);
        ...
        {
            var product = dbSet.Where(p => p.Id == id)
        ...
        {
            var toppings = dbSet.Where(t => t.IsTopping && t.StatusId == (int)EnumStatus.Active && t.StoreId == storeId && t.IsActive)
        ...
        public IQueryable<Product> GetAllToppingBelongToProduct(Guid storeId, Guid productId)
        {
            ...
            var toppings = dbSet.Where(t => t.StoreId == storeId && t.IsTopping && t.StatusId == (int)EnumStatus.Active && toppingIds.Contains(t.Id))
                .Include(t => t.ProductPrices);
        ...
        {
            var productExsited = await dbSet.AnyAsync(p => p.StoreId == storeId && p.Id != productId && p.Name.Trim().ToLower() == productName.Trim().ToLower());
        ...
        public async Task<Tuple<bool, int?, string>> UpdateProductAsync(Guid storeId, UpdateProductModel productEditModel)
        {
            ...
            using (var transaction = await _dbContext.Database.BeginTransactionAsync())
            {
                ...
                {
                    //Edit Product
                    var productEdit = _dbContext.Products.FirstOrDefault(x => x.Id == productEditModel.ProductId);
