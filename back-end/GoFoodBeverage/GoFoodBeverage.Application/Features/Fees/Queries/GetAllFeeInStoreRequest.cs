...
namespace GoFoodBeverage.Application.Features.Fees.Queries
{
    ...
    public class GetAllFeeInStoreRequestHandler : IRequestHandler<GetAllFeeInStoreRequest, GetAllFeeInStoreResponse>
    {
        ...
        private EnumFeeStatus GetFeeStatus(DateTime? startDate, DateTime? dueDate)
        {
            ...
            {
                var result = _dateTime.NowUtc.Date.CompareTo(startDate.Value.Date);
                ...
                else
                {
                    if (dueDate.HasValue)
                    {
                        return _dateTime.NowUtc.Date.CompareTo(dueDate.Value.Date) >= 0 ? EnumFeeStatus.Finished : EnumFeeStatus.Active;
                    }
                    else
                    {
                        return EnumFeeStatus.Active;
            ...
            {
                var currentDate = _dateTime.NowUtc.Date;
                ...
                {
                    return currentDate <= dueDate.Value.Date ? EnumFeeStatus.Active : EnumFeeStatus.Finished;
                }
                else
                {
                    if(currentDate == startDate.Value.Date)
                        return EnumFeeStatus.Active;
