...
namespace GoFoodBeverage.Application.Features.Materials.Commands
{
    ...
    public class CreateMaterialRequestHandler : IRequestHandler<CreateMaterialRequest, bool>
    {
        ...
        public async Task<bool> Handle(CreateMaterialRequest request, CancellationToken cancellationToken)
        {
            ...
            var materialInventoryBranches = CreateMaterialInventoryBranches(request, store.Id, newMaterial.Id);
        ...
        private static List<MaterialInventoryBranch> CreateMaterialInventoryBranches(CreateMaterialRequest request, Guid storeId, Guid materialId)
        {
            var newMaterialInventory = new List<MaterialInventoryBranch>();
            if (request.Branches.Any())
            {
                foreach (var inventory in request.Branches)
                {
                    newMaterialInventory.Add(new MaterialInventoryBranch()
                    {
                        StoreId = storeId,
