...
namespace GoFoodBeverage.Application.Features.Products.Queries
{
    ...
    public class GetProductCartItemRequestHandler : IRequestHandler<GetProductCartItemRequest, GetProductCartItemResponse>
    {
        ...
        private static void ClearDiscount(GetProductCartItemResponse response)
        {
            response.CartItems.Clear();
            response.OriginalPrice = 0;
            response.TotalPriceAfterDiscount = 0;
            if (response.DiscountTotalPromotion != null)
            {
                response.DiscountTotalPromotion.DiscountValue = 0;
            }
        }
        ...
        private static decimal CalculatingAllPriceAfterDiscount(decimal price, Promotion promotion, List<ProductCartItemModel.ToppingDto> toppingsOfItem)
        {
            ...
            if (promotion.IsPercentDiscount)
            {
                ...
                var maxDiscountValue = discountValue >= promotion.MaximumDiscountAmount ? promotion.MaximumDiscountAmount : discountValue;
            ...
            decimal totalOriginalPrice = 0m)
        {
            ...
            if (promotion != null)
            {
                ...
                if (promotion.PromotionTypeId == (int)EnumPromotion.DiscountProduct)
                {
